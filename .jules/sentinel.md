...
## 2026-05-31 - SSRF Bypass via Trailing Dots and IPv4-Compatible IPv6
**Vulnerability:** The `assertSafeTarget` URL guard logic failed to block IPv4-compatible IPv6 addresses like `[::127.0.0.1]` (which normalize to `[::7f00:1]`) and FQDN trailing dots (like `localhost.`). Both variants successfully evaded the `PRIVATE_HOSTS` blocklist, potentially allowing an SSRF attack against internal or private targets.
**Learning:** Node.js's `URL` parser retains FQDN trailing dots and automatically normalizes IPv4-compatible IPv6 addresses into compressed IPv6 formats, causing strict textual blocklist matches to fail.
**Prevention:** Always strip trailing dots (`hostname.replace(/\.$/, '')`) from extracted hostnames before applying validation. When protecting against IPv6 loopback variants, always block the normalized loopback space (`/^\[?::[0-9a-f:]+\]?$/i`) in addition to mapped variants.
