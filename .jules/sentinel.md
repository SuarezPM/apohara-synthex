...
## 2024-07-03 - SSRF Bypass via Trailing Dots and IPv6 Normalization
**Vulnerability:** The SSRF guard could be bypassed using trailing dots (e.g., `localhost.`) or IPv4-mapped IPv6 addresses (e.g., `[::127.0.0.1]`).
**Learning:** Node.js `URL` parser retains FQDN trailing dots, allowing exact regex matches like `/^localhost$/` to fail. It also normalizes IPv4-mapped IPv6 addresses (like `[::127.0.0.1]` to `[::7f00:1]`), bypassing text-based blacklist checks.
**Prevention:** Always strip trailing dots from `hostname` (`hostname.replace(/\.$/, '')`) before applying allowlist/blocklist checks. Add explicit blocklist rules for normalized IPv4-mapped IPv6 patterns.
