...
## 2026-05-30 - Model Output XSS Injection
**Vulnerability:** The classification output parsed from the LLM responses lacked sanitization. Malicious input strings crafted to return JSON payloads containing executable HTML (e.g., `<script>alert('XSS')</script>`) would be accepted by the schema and forwarded to downstream systems as classification summaries or signals.
**Learning:** Even though the frontend sanitizes output when rendering using `escapeHtml`, relying purely on client-side protections does not defend API consumers or backend systems receiving the evidence payloads. Data originating from untrusted models must be sanitized at the boundary.
**Prevention:** Apply an HTML-stripping function (`replace(/<[^>]*>/g, '')`) to the LLM's parsed output within the `parseClassification` function to ensure downstream consumers do not receive raw HTML tags.
