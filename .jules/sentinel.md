...
## 2025-05-31 - Cross-Site Scripting (XSS) via Attribute-Based Injection
**Vulnerability:** The `escapeHtml` helper in `public/index.html` failed to escape single quotes (`'`). While it successfully mitigated standard injection into the DOM (`<`, `>`), it left attribute-based XSS vulnerabilities open if unsanitized data was used inside a single-quoted HTML attribute.
**Learning:** Data originating from untrusted LLM outputs (e.g., classification summaries or signals) must be properly sanitized at the boundary. When performing client-side DOM manipulation by appending raw HTML to `innerHTML`, failing to escape single quotes can allow injection when the data lands in attributes like `onerror='...'` or similar event handlers.
**Prevention:** Always escape single quotes (`'`) to `&#39;` in inline HTML-escaping functions. Use `/[&<>"']/g` for replacement rather than `/[&<>"]/g` to ensure comprehensive mitigation of both tag-based and attribute-based XSS vectors.
