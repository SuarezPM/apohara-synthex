...
## 2026-06-21 - XSS Bypass via Single Quotes
**Vulnerability:** The `escapeHtml` helper in `public/index.html` failed to escape single quotes (`'`). This left variables interpolated into `innerHTML` attributes vulnerable to XSS.
**Learning:** The frontend's `escapeHtml` function correctly handled `&`, `<`, `>`, and `"`, but missed `'`. When user input is placed inside an HTML attribute delimited by single quotes, an attacker can break out of the attribute and inject an event handler (like `onmouseover`).
**Prevention:** Always escape single quotes (`'`) as `&#39;` when implementing custom `escapeHtml` helpers in vanilla JavaScript. Ensure that all data originating from untrusted outputs, including those from models, are sanitized.
