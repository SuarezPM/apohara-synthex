...
## 2026-05-30 - XSS via innerHTML Interpolation
**Vulnerability:** Variables interpolated into `innerHTML` via template literals without explicit HTML escaping were vulnerable to standard and attribute-based XSS if the underlying data originated from untrusted sources.
**Learning:** Even with an inline `escapeHtml` utility present, if it is not systematically applied to all interpolated variables, or if it misses critical characters like single quotes, XSS paths remain open.
**Prevention:** Always fortify escaping functions to include single quotes and ensure every variable interpolated into an `innerHTML` assignment is wrapped with the escaping function.
