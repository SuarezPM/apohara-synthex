...
## 2025-07-08 - SSRF blocklist bypass via trailing dots and IPv4-mapped IPv6

**Vulnerability:** The SSRF blocklist could be bypassed by appending a trailing dot to the hostname (e.g., `localhost.`) or by using IPv4-compatible/mapped IPv6 addresses (e.g., `[::127.0.0.1]` which parses to `[::7f00:1]`).
**Learning:** Node.js URL parser retains FQDN trailing dots and normalizes IPv4-mapped IPv6 addresses, which evade textual regex matches for standard loopback IPs.
**Prevention:** Always strip trailing dots (`u.hostname.replace(/\.$/, '')`) before applying allowlist/blocklist regexes, and explicitly block the normalized IPv6 mapped formats (`/^\[?::(?:ffff:)?7f[0-9a-f]{2}:[0-9a-f]{1,4}\]?$/i`) when doing regex-based SSRF defense.
