...
| **FORGE** | SHA-256 dedup + **two-layer deterministic pre-LLM defense — 28 web-injection (own) + 78 prompt-level (ported from [Aegis](https://github.com/SuarezPM/apohara-aegis), Apache-2.0)**. Layer 1 `prefilter.js` (28 rules): SSRF, prototype-pollution, MCP tool poisoning, indirect prompt-injection, BrowseSafe / VPI-Bench text vectors. Layer 2 `djl.js` (78 rules): prompt-injection, harm/PII, jailbreak, SQLi/XSS, exfiltration, tool misuse, sector policy (HIPAA/PCI/EO-13526). Audit trail per-stage emitted in payload v2 `decisions[]` with policy_bundle sha pin. |
...
- **Two-layer defense scope:** Synthex runs **28 own web-injection rules** (`src/forge/prefilter.js` — SSRF, prototype-pollution, MCP tool poisoning, indirect prompt-injection, BrowseSafe / VPI-Bench text vectors) **plus 78 prompt-level rules** ported from [Apohara Aegis](https://github.com/SuarezPM/apohara-aegis) (`src/forge/djl.js`, Apache-2.0, sha pin `f24d957`). Both layers are **heuristic regex deterministic** — *aligned with* the SkillFortify benchmark (arXiv 2603.00195), not a formal guarantee. They do **not** stop *visual* prompt injection (VPI in rendered screenshots/images) — a different threat model.
- **Parity vs Aegis source:** **78/78 fixtures pass identically** vs the original `apohara-aegis/tests/test_djl_rules.py @ f24d957` (verified by `test/djl.test.js`; divergences would be documented in [`docs/djl-parity-divergence.md`](docs/djl-parity-divergence.md) — currently empty). No claim "paridad formal": this is measured parity on 78 positive + 78 negative Aegis-curated fixtures, NOT on every possible adversarial input. FU-8 (post-hackathon): Pyodide-in-CI for strict parity vs real `djl.py` over the 156 fixtures without paying the Pyodide bundle in runtime.
- **Effective coverage on synthetic corpus** (SC-11, `node scripts/measure-coverage.mjs`): on the 156 Aegis fixtures (78 positive + 78 negative — a *synthetic* corpus, not real Bright Data scraping), DJL fires on **50.0% of docs** (the 78 positives, as expected) and **100% of the 78 rules** fired at least once; prefilter fires on **9.0% of docs** (natural overlap with DJL on SQLi/XSS/exf vectors) and **39.3% of the 28 rules**. On real Bright Data corpus the split will differ — prefilter higher (HTML scraping), DJL lower (rules designed for prompts, not docs). Re-run with `node scripts/measure-coverage.mjs <path-to-docs.json>` on your own corpus.
- **HMAC canonicalization (schema_v2):** since v4 the HMAC sealing uses `canonicalize()` (JCS-like, `src/prove/canonicalize.js`) so the order of payload keys is irrelevant — an identical v2 payload produces the same HMAC no matter how it was built. The verifier auto-detects `schema_version` and verifies both v1 (legacy, JSON.stringify) and v2 (canonicalize) without flags. Global flag `EVIDENCE_SCHEMA_V2=0` forces sealer legacy (rollback demo only; FU-7 removes it post-hackathon).
...
Synthex itself is **MIT** (this repo). It contains **one Apache-2.0 file** ported from a sibling open-source repo:

| File | License | Source |
|---|---|---|
| `src/forge/djl.js` | **Apache-2.0** | `apohara-aegis/apohara_aegis/djl.py` @ [`f24d957`](https://github.com/SuarezPM/apohara-aegis/commit/f24d957f7edc8e9054226c0d70a6adc617fa48f8) |
| `test/djl-fixtures.js` | **Apache-2.0** | `apohara-aegis/tests/test_djl_rules.py` @ same sha pin |
| Everything else | **MIT** | — |

The full Apache-2.0 license text from Aegis is preserved byte-exact in [`LICENSES/aegis-Apache-2.0.txt`](LICENSES/aegis-Apache-2.0.txt). The two licenses are compatible: Apache-2.0 code may be redistributed inside an MIT project as long as (a) the Apache-2.0 file keeps its SPDX header and origin pin, (b) the Apache LICENSE is reproduced, (c) this dual-licensing is documented (you're reading it). Aegis upstream has **no NOTICE file** (verified `find apohara-aegis -iname "NOTICE*"` → empty), so no NOTICE is reproduced — Apache-2.0 §4(d) requires reproducing NOTICE *if any*.

When Aegis advances, the sha pin and headers must bump together; the anti-drift assertion `test/djl.test.js` (T4 AC#N4) enforces `djl.js@sha === djl-fixtures.js@sha` mechanically.
