...
*Stats strip: 143 tests · 133 pass / 10 skip / 0 fail · RFC 3161 timestamp verifiable · 28-rule injection prefilter · 4 lenses in parallel · live UI at synthex.apohara.dev.*
...
**11 · Verify it yourself (60s).** `npm test` (143 tests · 133 pass / 10 skip / 0 fail) · `npm run demo` (signed report) · `npm run sample` (the sample PDF in `samples/`) · `check-pipeline-live.mjs … all` (live e2e, 4 lenses) · or the live UI **synthex.apohara.dev** (on Vercel). Public repo: github.com/SuarezPM/apohara-synthex. MIT.
