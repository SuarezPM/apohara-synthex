...
export async function runPipeline(target, opts = {}) {
  ...
  // ⚡ Bolt: O(1) Map lookup replacing O(N^2) array .find() to cross-reference safe docs
  const safeDocMap = new Map();
  for (const s of safe) {
    if (!safeDocMap.has(s.url)) safeDocMap.set(s.url, s);
  }

  for (const d of [...djlReviewed, ...prefReviewed]) {
    if (reviewBand.has(d.url) || !_isInjectionRow(d)) continue;
    const doc = safeDocMap.get(d.url);
