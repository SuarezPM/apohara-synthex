...
export async function runPipeline(target, opts = {}) {
  ...
  const { blocked, safe, dedup, tokensSaved, guardReviewed, djlReviewed, prefReviewed } = await timed("FORGE", async ({ record }) => {
    ...
    let safe = safe1;
    if (guardEnabled) {
      // ⚡ Bolt: Use bounded mapLimit instead of unbounded Promise.all to prevent
      // memory spikes and avoid hitting remote API rate limits when processing
      // large numbers of scraped documents concurrently.
