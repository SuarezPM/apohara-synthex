...
export async function runPipeline(target, opts = {}) {
  ...
  const { blocked, safe, dedup, tokensSaved, guardReviewed, djlReviewed, prefReviewed } = await timed("FORGE", async ({ record }) => {
    ...
    let safe = safe1;
    if (guardEnabled) {
      // Bounded concurrency (opt): Replaced unbounded Promise.all with mapLimit
      // to avoid hitting rate limits or memory exhaustion when screening large payloads.
      // Expected impact: More stable execution for large inputs, preserving resources.
