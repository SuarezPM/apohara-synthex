...
- **BLOCK authority is REVIEW-capped until item 1.2 measures FP.** A model `Unsafe` verdict is demoted to `review` unless `SYNTHEX_GUARD_BLOCK_ENABLED` is truthy (`_capVerdict`). The raw block-band score is always sealed so 1.2 can measure the benign FP before granting BLOCK. Fail-safe: an unmeasured guard never drops a scraped doc. // ES: la autoridad de BLOCK queda capada a REVIEW hasta que 1.2 mida el FP; el score crudo se sella igual.
