...
- **BLOCK authority gate — MEASURED 2026-05-29 (item 1.2).** A model `Unsafe` verdict is demoted to `review` unless `SYNTHEX_GUARD_BLOCK_ENABLED` is truthy (`_capVerdict`); the raw block-band score is always sealed. **Measured benign FP of `Qwen/Qwen3Guard-Gen-8B` on the 5-page corpus = 3/5 (60%), observed 2/5–3/5 across runs** — computed live by `node scripts/measure-guard-fp.mjs --provider=featherless` (0/5 unparsed; hosted inference is NOT run-to-run deterministic despite `temperature:0`; the FPs are page 01 (Simon Willison's prompt-injection blog), page 04 (PortSwigger SQLi), and the borderline page 05 (CVE-2021-44228, which flips `Safe`/`Unsafe` across runs), all quoting literal attack payloads — the two OWASP prevention cheat sheets classify `Safe`). **Decision rule ([`guard-fp-measurement.md`](guard-fp-measurement.md)):** a guard earns BLOCK only if benign FP ≤ 20% (the L1-heuristic bar). 60% > 20% (every observed run exceeds the bar) → **DISQUALIFIED.** **Recommended `SYNTHEX_GUARD_BLOCK_ENABLED=0` (leave unset) — BLOCK stays disabled for L2; every `Unsafe` degrades to REVIEW.** This is exactly the shipped default, so the measurement *confirms* the conservative fail-safe rather than changing it; **L3 AlignmentCheck (item 1.3) is the path to real BLOCK authority** (L2 = volume filter, L3 = FP-killer — an L2 FP that only REVIEWs is recoverable, an L2 BLOCK that drops a benign security page is not). Optional probes `google/shieldgemma-9b` + `OpenSafetyLab/MD-Judge-v0.1` → HTTP 404 on Featherless (**NOT MEASURED**, honest FAIL); `meta-llama/Llama-Guard-3-8B` is GATED → **NOT ATTEMPTED**. Fail-safe: an unmeasured/unqualified guard never drops a scraped doc. // ES: FP medido de Qwen3Guard-Gen-8B = **3/5 (60%)**, observado 2/5–3/5 entre corridas; regla: BLOCK sólo si FP ≤ 20%; 60% > 20% ⇒ NO califica; recomendado `SYNTHEX_GUARD_BLOCK_ENABLED=0` (sin setear) — BLOCK queda deshabilitado para L2 (todo a REVIEW), que es el default actual; L3 (1.3) es el camino a BLOCK real.

**v2.0.0 (P1.2) — two-axis FP+recall gate, and the multi-guard finding.** The item-1.2 number above is benign FP on a 5-page real corpus. v2 adds the mandatory SECOND axis — **recall** on a 647-sample LABELED corpus (`test/fixtures/guard-recall-corpus/`, constructed from OWASP LLM01 / ATLAS AML.T0051 / Bag-of-Tricks arXiv:2510.11570; describing-vs-executing labeled) — across the two open guards gate-confirmed live on Featherless (`Qwen3Guard-Gen-8B` + `nvidia/Llama-3.1-Nemotron-Safety-Guard-8B-v3`; Llama-Guard-3/2 are GATED, granite 404s). **Measured (2026-05-30, n=647, `node scripts/measure-guard-recall.mjs`): Qwen3Guard recall 90% but benign FP 35% (describing 60%) → DISQUALIFIED; NemoGuard recall 66% (all block-grade) at benign FP 11% (describing 16%) → QUALIFIES for BLOCK** (the ≤ 20% bar). So the multi-guard selection found a guard that EARNS BLOCK authority on its measured FP — NemoGuard. **Honest reading:** Qwen3Guard is the high-recall REVIEW-breadth filter; NemoGuard is the low-FP BLOCK gate; neither alone suffices (NemoGuard misses 34% of injections → L3 AlignmentCheck, measured false-BLOCK 0/5, backstops). **Cross-validation:** Qwen's 60% describing-FP on the constructed corpus matches its 3/5 on the real 5-page corpus — the constructed describing samples behave like real security writing. **Caveats:** constructed corpus (not in-the-wild); NemoGuard is binary (no REVIEW tier); hosted inference is non-deterministic; an unparsed reply counts as not-caught. **NemoGuard is now wired into the production `injection-guard.js` path** (commit 318f874) as an opt-in provider — BLOCK is still REVIEW-capped by default via `SYNTHEX_GUARD_BLOCK_ENABLED` (unset → every `Unsafe` degrades to REVIEW); the measurement above is what qualifies it for that opt-in BLOCK authority. Full table: [`docs/guard-recall-measurement.md`](guard-recall-measurement.md). // ES: segundo eje (recall) sobre 647 etiquetados; NemoGuard CALIFICA para BLOCK (FP 11% ≤ 20%, recall 66%), Qwen NO (FP 35%); medición, aún sin cablear a producción; NemoGuard binario, recall 66% → L3 respalda.
