...
- `src/guard.js assertSafeTarget` blocks RFC1918 + loopback + link-local. The egress path is Bright Data's remote proxy, so there is no `169.254.169.254` instance-metadata endpoint to reach from our process; DNS rebinding via Bright Data is outside our blast radius. The comment at `src/guard.js:6-10` already documents this — it lives here too so the caveat is one click from the pitch.
